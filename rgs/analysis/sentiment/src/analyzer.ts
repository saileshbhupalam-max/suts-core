...
import { validateEmotions, extractEmotions, type Emotion } from './emotions';
...
export class SentimentAnalyzer {
  ...
  private readonly config: Required<Omit<SentimentAnalyzerConfig, 'apiKey' | 'logger'>>;
  ...
  private async callClaudeAPI(content: string): Promise<SentimentResult> {
    try {
      ...
      if (textContent === undefined || textContent.type !== 'text') {
        throw new ScraperError('No text content in Claude response', 'sentiment', false);
      }
      ...
      const emotions =
        validEmotions.length > 0 ? validEmotions : extractEmotions(content, parsed.score);

      const result: SentimentResult = {
        score: parsed.score,
        magnitude: parsed.magnitude,
        emotions,
        confidence: this.calculateConfidence(parsed.score, parsed.magnitude, emotions),
      };
   ...
   */
  private async callClaudeBatchAPI(contents: string[]): Promise<SentimentResult[]> {
    try {
      ...
      return parsed.map(
        (item: z.infer<typeof SentimentResponseSchema>, index: number): SentimentResult => {
          const content = contents[index];
          if (content === undefined) {
            throw new ScraperError('Content index mismatch in batch response', 'sentiment', false);
          }

          const validEmotions = validateEmotions(item.emotions);
          const emotions =
            validEmotions.length > 0 ? validEmotions : extractEmotions(content, item.score);

          const result: SentimentResult = {
            score: item.score,
            magnitude: item.magnitude,
            emotions,
            confidence: this.calculateConfidence(item.score, item.magnitude, emotions),
          };
   ...
   */
  private parseClaudeResponse(response: string): z.infer<typeof SentimentResponseSchema> {
    try {
   ...
   */
  private parseBatchClaudeResponse(response: string): z.infer<typeof BatchSentimentResponseSchema> {
    try {
      // Remove markdown code blocks if present
      let cleaned = response.trim();
      if (cleaned.startsWith('```json')) {
   ...
   */
  private calculateConfidence(score: number, magnitude: number, emotions: Emotion[]): number {
  ...
  private handleAPIError(error: unknown): never {
    ...
    if (error instanceof Anthropic.APIError) {
      ...
      if (apiError.status === 529) {
        throw new ScraperError('Claude API is overloaded', 'sentiment', true, apiError);
      }

      // Authentication error (401)
      if (apiError.status === 401) {
        throw new ScraperError('Invalid Anthropic API key', 'sentiment', false, apiError);
      }
    ...
    if (error instanceof Error && error.message.includes('network')) {
      throw new ScraperError('Network error calling Claude API', 'sentiment', true, error);
    }
