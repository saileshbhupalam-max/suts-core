...
export function validateSourceConfig(config: SourceConfig): boolean {
  ...
  if (config.maxItems !== undefined && (typeof config.maxItems !== 'number' || config.maxItems <= 0)) {
    return false;
  }

  if (config.timeRangeHours !== undefined && (typeof config.timeRangeHours !== 'number' || config.timeRangeHours <= 0)) {
    return false;
  }
