...
describe('ReportGenerator', () => {
  ...
  describe('generate', () => {
    ...
    it('should respect minThemeFrequency option', async () => {
      ...
      const allThemesAboveMin = result.summary.topThemes.every((theme) => theme.frequency >= 30);
  ...
  describe('validation', () => {
    it('should throw error if data has no signals', async () => {
      ...
      await expect(generator.generate(invalidData, options)).rejects.toThrow(ReportGenerationError);
    });
    ...
    it('should throw error if outputDir is empty', async () => {
      ...
      await expect(generator.generate(mockData, options)).rejects.toThrow(ReportGenerationError);
    });
