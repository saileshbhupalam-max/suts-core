...
export class ReportGenerator {
  ...
  private validateData(data: ReportData): void {
    try {
      ...
      if (Math.abs(sum - 1.0) > 0.01) {
        throw new Error(`Sentiment distribution must sum to 1.0, got ${sum.toFixed(2)}`);
      }
    } catch (error) {
      throw new ReportGenerationError(
        'Invalid report data',
   ...
   */
  private async generateMarkdown(data: ReportData, options: ReportOptions): Promise<string> {
    try {
   ...
   */
  private filterThemes(themes: ReportData['themes'], options: ReportOptions): ReportData['themes'] {
